...
DEFAULT_WEBHOOK_URLS = {
    'BTC': 'https://api.primeautomation.ai/webhook/ChartPrime/50230c08-23a8-4e85-8da9-870d61a0cd07',
    'SOL': 'https://api.primeautomation.ai/webhook/ChartPrime/dad24c0c-0829-40cf-8b18-e65a5a8ba09a'
}
DEFAULT_WEBHOOK_URL = 'https://api.primeautomation.ai/webhook/ChartPrime/32e8383b-c393-4dec-939d-f4345d713b01'
    ...
    def from_file(cls, filename: str) -> 'Config':
        ...
        try:
            ...
            cleaned_config = {
                ...
                'webhook_enabled': config_data.get('webhook_enabled', True),  # Default to enabled
...
class TradeSignal:
    ...
    per: Optional[float] = None  # Percentage return
...
class Position:
    ...
    def to_exit_signal(self, current_price: float, reason: str) -> 'TradeSignal':
        ...
        # For webhook, we use max(0, profit_pct) to avoid negative values
        return TradeSignal(
            ticker=self.ticker,
            action=exit_action,
            price=current_price,
            ...
            per=profit_pct,  # Store actual value, the webhook formatter will handle displaying 0 instead of negative
            reason=reason
        )
...
class TradingAdvisor:
    ...
    def _parse_signal(self, response_text: str, ticker: str, current_price: float) -> Optional[TradeSignal]:
        ...
        return TradeSignal(
            ...
            size=size,
            sl=None,  # Will be calculated by the position manager
            tp=None,  # Will be calculated by the position manager
            rationale=reason,
...
# DIRECT COPY FROM THE SECOND SCRIPT - Signal Dispatcher implementation
class SignalDispatcher:
    """
    Enhanced system for reliable signal delivery - Directly copied from second script
    """
    def __init__(self):
    ...
    def send_webhook(self, symbol, action, price, **kwargs):
        ...
        if action == "buy":
            ...
            # For exit signals, ensure per value is never negative (use 0% instead)
...
class CryptoMCP:
    ...
    def __init__(self, config_file: str):
        ...
        # Initialize signal dispatcher - NOT using config here, using direct constants
    ...
    def process_ticker(self, ticker: str):
        ...
        try:
            ...
            # For testing, force to not have open positions to generate signals
            ...
            if signal:
                if signal.action in ["buy", "sell"]:
                    ...
                    # Send webhook using the direct method from second script
...
if __name__ == "__main__":
    ...
    try:
        ...
        config_file = "updated-confi.json"
        ...
        for ticker in ["BTC", "SOL"]:
            print(f"\nForcing test signal for {ticker}...")
            current_price = mcp.market_provider.get_market_data(ticker).price
            
            # Test sending a buy signal
            success = mcp.dispatcher.send_webhook(
                symbol=ticker,
                action="buy",
                price=current_price,
                confidence="0.85"
            )
            print(f"Buy signal sent successfully: {success}")
            
            # Test sending an exit signal (exit a buy)
            success = mcp.dispatcher.send_webhook(
                symbol=ticker,
                action="exit_buy",
                price=current_price * 1.01,  # Simulating 1% profit
                size="1",
                per=1.0,  # 1% profit
            ...
            print(f"Exit signal sent successfully: {success}")
