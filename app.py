        ...
        if st.session_state.positions:
            ...
            for pos_id, pos in st.session_state.positions.items():
                if isinstance(pos, dict):  # Make sure it's a valid position dict
                    ...
                    symbol_display = f"{pos.get('symbol', '')} {'⚠️ Test' if pos.get('is_test', False) else ''}"
            ...
            if trades:
                ...
                for trade in recent_trades:
                    if isinstance(trade, dict):  # Make sure it's a valid trade dict
                        ...
                        symbol_display = f"{trade.get('symbol', '')} {'⚠️ Test' if trade.get('is_test', False) else ''}"
                        trade_data.append({
                            "Symbol": symbol_display,
            ...
            with st.spinner("Running diagnostics..."):
                ...
                try:
                    ...
                    if 'accounts' in balance and balance['accounts']:
                        st.success("✅ Account API connection successful")
                        st.json(balance)
                    else:
                        st.error("❌ Account API connected but no data returned")
                except Exception as e:
                    st.error(f"❌ Failed to connect to account API: {e}")
                ...
                try:
                    ...
                    if allowed_coins:
                        ...
                        if ticker and 'last' in ticker and ticker['last'] > 0:
                            st.success(f"✅ Price data available for {sample_symbol}: {ticker['last']}")
                            
                            # Check trading criteria
                            price_data = st.session_state.bot.fetch_market_data(sample_symbol)
                            if not price_data.empty:
                                st.success(f"✅ Historical price data available for {sample_symbol}")
                                ...
                                if abs(pct_change) >= trigger_pct:
                                    st.info(f"💡 This would trigger a buy signal for {sample_symbol}")
                                else:
                                    st.info(f"⏸️ No buy signal for {sample_symbol} based on current change")
                            else:
                                st.error("❌ Could not fetch historical price data")
                        else:
                            st.error(f"❌ Could not get price data for {sample_symbol}")
                    else:
                        st.error("❌ No tradeable instruments found")
                ...
                try:
                    if not st.session_state.bot.client.api_key or len(st.session_state.bot.client.api_key) < 10:
                        st.warning("⚠️ API key appears to be missing or too short")
                    else:
                        masked_key = st.session_state.bot.client.api_key[:5] + "..."
                        st.success(f"✅ API key present: {masked_key}")
                    
                    if not st.session_state.bot.client.secret or len(st.session_state.bot.client.secret) < 10:
                        st.warning("⚠️ API secret appears to be missing or too short")
                    else:
                        st.success("✅ API secret is present")
                except Exception as e:
                    st.error(f"❌ Error checking API credentials: {e}")
    else:
        st.warning("⚠️ Bot is not running. Start the bot to run diagnostics.")
        ...
        if st.button("Check API Configuration"):
            ...
            if st.session_state.config:
                ...
                if not secret or len(secret) < 10:
                    st.error("❌ API secret is missing or invalid")
                else:
                    st.success("✅ API secret is configured")
            else:
