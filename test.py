...
class Config:
    ...
    check_tp_sl_interval: int = 60  # Check take profit/stop loss every minute (reduced from 500 seconds)
    position_check_interval: int = 30  # Check positions every 30 seconds (reduced from 60 seconds)
    ...
    def from_file(cls, filename: str) -> 'Config':
        ...
        try:
            ...
            cleaned_config = {
                ...
                'check_tp_sl_interval': config_data.get('check_tp_sl_interval', 60),
                'position_check_interval': config_data.get('position_check_interval', 30)
            }
...
class Position:
    ...
    def to_exit_signal(self, current_price: float, reason: str) -> 'TradeSignal':
        ...
        # Store the actual profit percentage (can be negative)
        # The signal dispatcher will handle displaying 0 instead of negative values
        return TradeSignal(
            ticker=self.ticker,
            action=exit_action,
            price=current_price,
            ...
            per=profit_pct,  # Actual percentage (can be negative)
            reason=reason
        )
...
class PositionManager:
    ...
    def _calculate_tp_distance(self, ticker: str, price: float) -> float:
        ...
        if ticker == "BTC":
            return 50  # $200 for BTC
        elif ticker == "ETH":
            return 100   # $100 for ETH
        elif ticker == "SOL":
            return 1.5     # $3 for SOL
        else:
...
# Signal Dispatcher with position tracking
    ...
    def send_webhook(self, symbol, action, price, **kwargs):
        ...
        if action == "buy":
            payload = {
                ...
                "time": current_time,
                "duration": "11.2h",
                "confidence": str(kwargs.get("confidence", "0.8")),
                "regime": "Ranging"
            }
            message = "BUY"
        elif action == "sell":
            ...
            # Get per value from kwargs
            per_value = kwargs.get("per", 0)
            
            # Calculate correct percentage for profit
            if per_value is not None:
                # Ensure per_value is non-negative for display
                if per_value < 0:
                    display_per = 0
                else:
                    display_per = per_value
            else:
                display_per = 0
            ...
            payload = {
                ...
                "per": f"{display_per:.1f}%",  # Show one decimal place for better accuracy
...
if __name__ == "__main__":
    ...
    try:
        ...
        config_file = "C:\\path\\alex\\ai-agent\\first-test\\updated-confi.json"
